...
export class SprintAnalyticsService {
  ...
  async getDashboard(org_id: string): Promise<SprintDashboardData> {
    ...
    const sprintSummaries = await Promise.all(
      sprints.map((s) => this.buildSprintSummary(s)),
    );

    const activeSprint = sprints.find((s) => s.status === 'active');
    let currentSprint: SprintDetail | null = null;
    if (activeSprint) {
      currentSprint = await this.buildSprintDetail(activeSprint);
    }

    const velocityTrend = await this.buildVelocityTrend(sprints);
    const burndown = activeSprint
      ? await this.buildBurndown(activeSprint)
      : [];
    const teamPerformance = activeSprint
      ? await this.buildTeamPerformance(activeSprint)
      : [];
  ...
  async getSprintDetail(org_id: string, sprint_id: string): Promise<SprintDetail> {
    ...
    return this.buildSprintDetail(sprint);
  }
  ...
  async getSprintBurndown(org_id: string, sprint_id: string): Promise<BurndownPoint[]> {
    ...
    if (!sprint) throw new NotFoundException('Sprint not found');
    return this.buildBurndown(sprint);
  }
  ...
  async getVelocityComparison(org_id: string): Promise<VelocityPoint[]> {
    ...
    return this.buildVelocityTrend(sprints);
  }

  // --- Private Helpers ---

  private async buildSprintSummary(sprint: Sprint): Promise<SprintSummary> {
  ...
  private async buildSprintDetail(sprint: Sprint): Promise<SprintDetail> {
    const summary = await this.buildSprintSummary(sprint);
    const tasks = await this.taskRepository.find({ where: { sprint_id: sprint.id } });
    ...
    // Average completion time (hours from creation to done)
    const doneTasks = tasks.filter((t) => t.status === 'done');
    const avgCompletionTime = doneTasks.length
      ? doneTasks.reduce((sum, t) => {
          const created = new Date(t.created_at).getTime();
          const estimate = t.estimated_hours || 0;
          return sum + estimate;
        }, 0) / doneTasks.length
      : 0;
    ...
    return {
      ...
      completion_rate: tasks.length ? (doneTasks.length / tasks.length) * 100 : 0,
      average_completion_time_hours: avgCompletionTime,
    };
  }

  private async buildVelocityTrend(sprints: Sprint[]): Promise<VelocityPoint[]> {
    return Promise.all(
      sprints.map(async (s) => {
        const tasks = await this.taskRepository.find({ where: { sprint_id: s.id } });
  ...
  private async buildBurndown(sprint: Sprint): Promise<BurndownPoint[]> {
    const tasks = await this.taskRepository.find({ where: { sprint_id: sprint.id } });
    const totalPoints = tasks.reduce((sum, t) => sum + (t.story_points || 0), 0);
    ...
    for (let day = 0; day <= daysElapsed; day++) {
      ...
      // For past days, estimate remaining based on completion
      const remainingPoints = Math.max(
        0,
        totalPoints - (day / daysTotal) * totalPoints * (tasks.filter((t) => t.status === 'done').length / Math.max(1, tasks.length)),
      );

      points.push({
        day,
        date: date.toISOString().split('T')[0],
        remaining_points: Math.round(remainingPoints * 100) / 100,
  ...
  private async buildTeamPerformance(sprint: Sprint): Promise<TeamMemberPerformance[]> {
    const tasks = await this.taskRepository.find({ where: { sprint_id: sprint.id } });
    ...
    for (const uid of userIds) {
      ...
      result.push({
        user_id: uid,
        user_name: uid, // Will be enriched by frontend
