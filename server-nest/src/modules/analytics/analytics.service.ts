...
export class AnalyticsService {
    ...
    const analytics = await Promise.all(
      projects.map(async (project) => {
        ...
        return {
          ...
          efficiency: totalEstimatedHours
  ...
  async getTimeTracking(org_id: string, user_id?: string, project_id?: string) {
    ...
    const analytics = {
      ...
      average_logged_per_task: tasks.length
        ? tasks.reduce((sum, t) => sum + (t.time_logged || 0), 0) / tasks.length
        : 0,
      efficiency_rate: tasks.reduce(
        (sum, t) => sum + (t.estimated_hours || 0),
        0,
      )
  ...
  async getAllKPIs(org_id: string, filters?: any) {
    ...
    if (filters?.category) {
      query.andWhere('kpi.category = :category', {
        category: filters.category,
      });
    }

    if (filters?.owner_id) {
      query.andWhere('kpi.owner_id = :owner_id', {
